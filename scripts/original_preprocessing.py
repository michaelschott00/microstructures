...
def crop_segmentation(input_dir, output_dir, crop_size, img_size):
    ...
    for original_file, mask_file in zip(original_files, mask_files):
        ...
        for i in range(0, img_size, crop_size):
            for j in range(0, img_size, crop_size):
                ...
                original_crop.save(
                    os.path.join(output_dir, "Original", f"{original_file}_{i}_{j}.png")
                )
                mask_crop.save(
                    os.path.join(output_dir, "Masks", f"{mask_file}_{i}_{j}.png")
                )
        ...
        for img_file in os.listdir(os.path.join(input_dir, class_name)):
            ...
            for i in range(0, img_size[0], crop_size):
                for j in range(0, img_size[1], crop_size):
                    ...
                    cropped_img.save(
                        os.path.join(output_dir, class_name, f"{filename}_{i}_{j}{ext}")
                    )
...
def crop_new_images(input_dir, output_dir, crop_size, img_size):
    ...
    for original_file, mask_file in zip(original_files, mask_files):
        ...
        assert original_img.size == mask_img.size == (2776, 2080), (
            f"Original: {original_img.size}, Mask: {mask_img.size}"
        )
    ...
    for folder in folders:
        ...
        for file in train_files:
            os.rename(
                ...
                os.path.join(output_dir, "train", folder, file),
            )
        for file in dev_files:
            os.rename(
                os.path.join(input_dir, folder, file),
                os.path.join(output_dir, "dev", folder, file),
            )
        for file in test_files:
            os.rename(
                os.path.join(input_dir, folder, file),
                os.path.join(output_dir, "test", folder, file),
            )
...
def segmentation_split(input_dir, output_dir, train_size, dev_size, test_size):
    ...
    for image, mask in train_files:
        os.rename(
            os.path.join(input_dir, "Original", image),
            os.path.join(output_dir, "train", "Original", image),
        )
        os.rename(
            os.path.join(input_dir, "Masks", mask),
            os.path.join(output_dir, "train", "Masks", mask),
        )
    for image, mask in dev_files:
        os.rename(
            os.path.join(input_dir, "Original", image),
            os.path.join(output_dir, "dev", "Original", image),
        )
        os.rename(
            os.path.join(input_dir, "Masks", mask),
            os.path.join(output_dir, "dev", "Masks", mask),
        )
    for image, mask in test_files:
        os.rename(
            os.path.join(input_dir, "Original", image),
            os.path.join(output_dir, "test", "Original", image),
        )
        os.rename(
            os.path.join(input_dir, "Masks", mask),
            os.path.join(output_dir, "test", "Masks", mask),
        )
...
def main():
    ...
    subparsers = parser.add_subparsers(
        title="subcommands", help="choose one of the following subcommands"
    )

    # cropping
    crop_parser = subparsers.add_parser(
        name="crop", help="crop images into smaller images"
    )
    crop_parser.add_argument(
        "--crop_size",
        type=int,
        default=256,
        help="size of the cropped images, one side, in pixels",
    )
    crop_parser.add_argument(
        "--img_size",
        nargs="+",
        type=int,
        help="size of the original images, one side, in pixels",
    )
    crop_parser.add_argument(
        "--input_dir", type=str, default=os.path.join("data", "segmentation_1", "raw")
    )
    crop_parser.add_argument(
        "--output_dir",
        type=str,
        default=os.path.join("data", "segmentation_1", "tiled"),
    )
    crop_parser.add_argument("--new", type=bool, default=False)

    # splitting
    split_parser = subparsers.add_parser(
        name="split", help="split images into train, dev and test sets"
    )
    split_parser.add_argument(
        "--train_size",
        type=float,
        default=0.8,
        help="size of the train set, between 0 and 1",
    )
    split_parser.add_argument(
        "--dev_size",
        type=float,
        default=0.1,
        help="size of the dev set, between 0 and 1",
    )
    split_parser.add_argument(
        "--test_size",
        type=float,
        default=0.1,
        help="size of the test set, between 0 and 1",
    )
    split_parser.add_argument(
        "--seed", type=int, default=42, help="seed for the random number generator"
    )
    split_parser.add_argument("--input_dir", type=str)
    ...
    assert os.path.isdir(args.input_dir), (
        f"input directory {args.input_dir} is not a directory"
    )
    if "crop_size" in args:
        assert (
            args.crop_size <= args.img_size[0] and args.crop_size <= args.img_size[1]
        ), f"crop size {args.crop_size} is larger than image size {args.img_size}"
        assert "tiled" not in os.listdir(args.output_dir), "data is already tiled"
    if "train_size" in args:
        assert args.train_size + args.dev_size + args.test_size == 1, (
            "train, dev and test size must sum up to 1"
        )
        assert not any(
            x in os.listdir(args.output_dir) for x in ["train", "test", "dev"]
    ...
    if "crop_size" in args:
        if args.new:
            crop_new_images(
                args.input_dir, args.output_dir, args.crop_size, args.img_size
            )
        ...
        if "classification" in args.input_dir:
            ...
            raise ValueError(
                "input directory must contain either 'classification' or 'segmentation'"
            )
